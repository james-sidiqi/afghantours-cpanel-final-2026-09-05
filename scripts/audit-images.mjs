...
for(const f of files){ const txt=fs.readFileSync(f,'utf8'); for(const m of txt.matchAll(re)){ const ref=m[0].replace(/[.,;]+$/,''); if(ref.includes('${')) continue; if(!refs.has(ref)) refs.set(ref,[]); refs.get(ref).push(path.relative(root,f)); }}
let missing=[];
