---
title: "Cycling"
activity_code: "ACT-CYC"
subtitle: "Urban and near-hub cycling around Kabul when conditions allow."
card_description: "Cycling as a soft-adventure option from the Kabul hub — route and timing confirmed case by case."
best_season: "Spring–autumn"
duration: "1–3 hours"
hero_image: "/assets/images/experiences/activities/cycling/hero.webp"
---

Cycling is currently locked to the **Kabul** hub as a soft-adventure option when traffic, security posture, and route conditions allow.

## Soft adventure

This is not a mountain-bike expedition catalog. It is a near-hub cycling option confirmed case by case during planning.

## Honest rules

No invented bike-rental shops, route apps, or package prices appear here. If a workable outing exists, it is scoped with local guidance.

## Expansion

Additional hubs are not invented for this task. Future hub links require operator review.
